			...
			if ( $transaction instanceof EE_Transaction ) {
				...
				if ( $this->_can_easily_itemize_transaction_for($payment) ) {
					...
					foreach ( $total_line_item->get_items() as $line_item ) {
						$l_item = array(
							'L_NAME' . $item_num => substr($line_item->name(), 0, 127),
							'L_DESC' . $item_num => substr($line_item->desc(), 0, 127),
					...
					array_push($order_l_items, array(
						'L_NAME1' => sprintf( __("Payment for registration: %s", 'event_espresso'), $primary_registrant->reg_code() ),
						'L_DESC1' => sprintf( __("Payment of %s for %s", 'event_espresso'), $payment->amount(), $primary_registrant->reg_code() ),
