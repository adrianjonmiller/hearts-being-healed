<?php wp_enqueue_script( 'gam-event-manager-ajax-filters' ); ?>
	...
	<?php do_action( 'event_manager_event_filters_search_events_start', $atts ); ?>
		 <div class="row">
			...
			<div class="col-sm-4">
				...
				<label for="search_keywords"><?php _e( 'Keywords', 'gam-event-manager' ); ?></label>
				<input type="text" name="search_keywords" id="search_keywords" placeholder="<?php esc_attr_e( 'Keywords', 'gam-event-manager' ); ?>" value="<?php echo esc_attr( $keywords ); ?>" /> 
			...
			<div class="col-sm-4">
				<label for="search_location"><?php _e( 'Location', 'gam-event-manager' ); ?></label>
				<input type="text" name="search_location" id="search_location"  placeholder="<?php esc_attr_e( 'Location', 'gam-event-manager' ); ?>" value="<?php echo esc_attr( $location ); ?>" />
			</div>
			...
			<?php if ( $datetimes) : ?>				
				<div class="col-sm-4">
					<label for="search_datetimes"><?php _e( 'Any dates', 'gam-event-manager' ); ?></label>
		...
		<div class="row">
			...
			<?php elseif ( $show_categories && ! is_tax( 'event_listing_category' ) && get_terms( 'event_listing_category' ) ) : ?>
				<div class="col-sm-4">
					<label for="search_categories"><?php _e( 'Category', 'gam-event-manager' ); ?></label>
					...
					<?php else : ?>
						<?php event_manager_dropdown_selection( array( 'taxonomy' => 'event_listing_category', 'hierarchical' => 1, 'show_option_all' => __( 'Any Category', 'gam-event-manager' ), 'name' => 'search_categories', 'orderby' => 'name', 'selected' => $selected_category, 'multiple' => false, 'hide_empty' => false) ); ?>
					<?php endif; ?>
				</div>
			<?php endif; ?>	 
			...
			<?php elseif ( $show_event_types && ! is_tax( 'event_listing_type' ) && get_terms( 'event_listing_type' ) ) : ?>		
				<div class="col-sm-4">
					<label for="search_event_types"><?php _e( 'Event Type', 'gam-event-manager' ); ?></label>
					...
					<?php else : ?>
						<?php event_manager_dropdown_selection( array( 'taxonomy' => 'event_listing_type', 'hierarchical' => 1, 'show_option_all' => __( 'Any Event Type', 'gam-event-manager' ), 'name' => 'search_event_types', 'orderby' => 'name', 'selected' => $selected_event_type, 'multiple' => false,'hide_empty' => false) ); ?>
					<?php endif; ?>
				</div>
			<?php endif; ?>		        
			...
			<?php if ( $show_ticket_prices && $ticket_prices) : ?>				
				<div class="col-sm-4">
					<label for="search_ticket_prices"><?php _e( 'Ticket Prices', 'gam-event-manager' ); ?></label>
...
<noscript><?php _e( 'Your browser does not support JavaScript, or it is disabled. JavaScript must be enabled in order to view listings.', 'gam-event-manager' ); ?></noscript>